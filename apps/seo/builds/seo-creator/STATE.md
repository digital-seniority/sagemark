...
**Last updated:** 2026-06-27 (**v1 BUILD COMPLETE** — Phase 0 + Phase 1 all merged; **P1.C.4 DoD CLOSED** with 30 real labeled `share_of_model` rows. DR-026 data-layer (read #66 / **write #67 MERGED** / **review #69 MERGED**), SoM #72, activation #74 all MERGED + INERT on preview. **0 open PRs.** **audit-006 done (no Critical, go-live-ready).** Now in the James-directed **audit+harden** unattended block (Run #24/25): correctives A.006.x being built — incl. **H1 go-live functional blocker** (the credentialed-release writer is unwired → after the env flip, no route can record a YMYL release; A.006.1, held for human merge). Go-live + real reviewer + pilot workspace remain HUMAN-gated; Phase 2 (GA) is OUT OF v1 scope (prd.md:714).)
...
## Next up (audit+harden block, Run #24/25 — A.006.x correctives, NEVER auto-merge)

- **A.006.1 (HIGH — go-live functional blocker) — HOLD FOR HUMAN:** wire `recordCredentialedRelease`/`recordClientSignoff` into the review→release route with `pilot: isPilot()` + test (release persisted on approval; `placeholder-in-production` refusal in prod). Folds in M3 (unify `defaultPublishEnabled`→`activationPublishEnabled`). Production-critical (publish/DR-037) → human merge.
- **A.006.2 (MED):** extend `gate-path-lint` discovery to model-resolving files under `packages/core/src/ai/` (covers `som-direct-runner.ts` — the 2nd metered Gateway call site) with the `forceGateway`-required assertion.
- **A.006.3 (MED):** add `runtime="nodejs"`/`dynamic="force-dynamic"`/`maxDuration` to the two cron routes (convention + runtime safety).
- **A.006.4 (MED, docs):** reconcile RFC §3.1 + PRD §3.4/§16 + `0039` comments + `content.ts` to the SoM HYBRID channel model (`direct-citation`/`direct-proxy`/`vendor` + proxy≠citation rule); fix the live-adapter "not wired" file headers → "wired, creds-gated."
- **Decision records written (this landing):** [[DR-040]] (activation gate model), [[DR-041]] (live SoM ingest / DoD-close method). Manifest check added: `live-side-effect-idempotency`.
- **HUMAN-GATED (not buildable unattended):** go-live env flip (`go-live-checklist.md`); a REAL credentialed reviewer replacing the placeholder (the H1 release-write path is built by A.006.1 but the *reviewer identity* is human); pilot workspace; GEO-tracker vendor (SoM proxy→real-citation upgrade); live-Sandbox Stage B/C (bridge-JWT secret + worker Gateway cred + Sandbox provisioning).
