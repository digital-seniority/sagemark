...
## Attended — #56/#58 merged + migrations applied + A.005.2 — 2026-06-26
- **P1.C.2 #56 MERGED** (`cab8827`) + **P1.C.3 #58 MERGED** (`9f58e62`, rebased over #56's shared-file conflicts, both-sides additive resolution, re-verified 491 pass). **Phase 1: 11/12 — only P1.C.4 remains.**
- **Migrations 0038 + 0039 APPLIED to Supabase + verified** (placeholder column + 3 ledger tables; RLS fail-closed). All 0036-0039 now live. Pilot reviewer seed pending pilot-workspace provisioning (needs its uuids).
- **A.005.2 [#61](https://github.com/digital-seniority/sagemark/pull/61) OPEN** (audit-finding, human-merge): real anon-isolation Tier-2 for `token-scope` (was `assert.ok(true)` — vacuous once `DATABASE_URL` set; now grants anon SELECT then asserts 0 rows; `psqlAvailable()` → clean skip not vacuous-pass; runs live in CI) + `gate-path-lint` glob-discovers all gate files (zero-discovered tripwire + new-gate test). Judge APPROVED 5/5·5/5. Closes audit-005 H3/H4/M5.
- **Next correctives:** A.005.1 (PersistedAuthorization `granted_at`/`scope` per [[DR-039]] + wire `recordCredentialedRelease` `pilot:false`), A.005.3 (spec-update + mount the 2 Inspector panels). **Then P1.C.4** (needs the Whispering Willows SoM prompt-set) = Phase-1 close.

## Run #23 (UNATTENDED, James-directed 5h) — iteration 1: audit-005 correctives — 2026-06-26
- **A.005.2 [#61](https://github.com/digital-seniority/sagemark/pull/61) MERGED** (`463240e`): real anon-isolation Tier-2 (now runs LIVE + passes in CI) + gate-path-lint glob-discovery. **Caught + fixed a CI race** ("tuple concurrently updated" — rls-contract + token-scope both `CREATE ROLE anon`/`GRANT` concurrently): serialized `node --test --test-concurrency=1` + dropped the redundant turbo `DATABASE_URL` passthrough (Tier-2 runs once, in the dedicated step). Judge 5/5·5/5.
- **A.005.1 [#63](https://github.com/digital-seniority/sagemark/pull/63) MERGED** (`adab32d`): `PersistedAuthorization` widened (`granted_at`/`scope`) + §11.5 predicate strengthened (granted ∧ ¬revoked ∧ ¬expired ∧ in-scope; fail-closed on every degenerate input; read/write parity via the single shared `isAuthorizationActive`). No migration. Judge 5/5·5/5. **[[DR-025]] DISCHARGED.**
- **A.005.3 [#64](https://github.com/digital-seniority/sagemark/pull/64) MERGED** (`aa7bc47`): RFC/PRD reconciled to shipped (gate_results=seam projection [[DR-039]]; `0039`+`seo_cost_run_budget`; 3-engine SoM [[DR-038]]) + mounted the 2 Inspector panels (data-gated, degrade-safe) + smoke tests. Judge 5/5·5/5. **anchors.json refreshed** (prd+rfc → run 23). DR-037 `scope` prose corrected (ymyl→client).
- **Next (iteration 2): the DR-026 live `ContentDataAccess` adapter** — the big production data-wiring (read methods → write methods → wire signoff (M1, `pilot:false`) + ledger writers; + the `sse-relay.ts` stale gate_results comment cleanup). Then terminal on non-eng blockers (P1.C.4 SoM prompt-set, pilot-workspace seed, live-Sandbox).

## Run #23 iter 2-3 — DR-026 live data-layer (read merged, write open) + M6 — 2026-06-26
- **DR-026 READ adapter [#66](https://github.com/digital-seniority/sagemark/pull/66) MERGED** (`4aaeeec`): live service-role `ContentDataAccess`/`PublicContentDataAccess` READ methods (14), every query arg-bound tenancy-filtered (RLS bypassed → app filter is the boundary), fail-closed mapping, INERT (factory-only, no route, type-level read-only). `getAuthorization` carries the A.005.1 granted_at/scope shape; `getGateResult` is the DR-039 seam projection. Folded the sse-relay gate_results comment fix. Judge 5/5·5/5. Auto-merged (inert foundation, zero live behavior change).
- **DR-026 WRITE adapter [#67](https://github.com/digital-seniority/sagemark/pull/67) OPEN for human review** (`worktree-agent-a75bd3478fd712b78`, inert): 6 live writes + 2 fail-closed (`nameVersion`/`setActiveVersion` — columns don't exist). Tenancy-bound (cross-tenant id → 0 rows, non-vacuous tests); **no gate/FSM bypass** (pure persistence; gate runs upstream in publish/route `assertTransition` + signoff `isAuthorizationActive`/DR-037); **DR-031 immutability** (release INSERT-only, `UNIQUE(piece,version)` rejects dup, no named-version write path). Judge 5/5·5/5. **NOT auto-merged — the mutation layer gets James's eyes.**
- **M6 RESOLVED:** `worktree-base-freshness` manifest structured-check added (formalizes the DR-036 ff-guard).
- **Next (iter 4): the `ReviewCommentDataAccess` live adapter** (api/review/comments seam — the last NOT_WIRED data-layer gap), inert. Then TERMINAL: the live-pipeline **activation** (route-wiring + creds + `pilot:false` + publishEnabled) is the human go-live gate; P1.C.4 needs the SoM prompt-set; pilot seed needs the pilot workspace; live-Sandbox Tier-2/3 needs infra.

## Run #23 iter 4 + TERMINAL — DR-026 review-token/comment adapter; loop complete — 2026-06-26
- **DR-026 review-token/comment adapter [#69](https://github.com/digital-seniority/sagemark/pull/69) OPEN for human review** (inert): `resolveTokenByHash` = the #1 cross-tenant boundary (full-hash unique-index match + revoked/expired filters → tuple or null, fail-closed; forged/revoked/expired/cross-tenant → null, non-vacuous); `resolvePreviewTarget` exposure-safe (only the 5 review-safe fields, no scorecard/credits/cost/model/body); `insertComment` binds tenancy from the resolved tuple. Judge 5/5·5/5. **NOT auto-merged (token boundary).** Closes the last DR-026 gap → the inert live data layer is COMPLETE.
- **⏹ Run #23 TERMINAL (~3h/5h):** eligible safe dependency-free engineering depleted. **Did NOT manufacture marginal blocked-lane work** (e.g. build:worker dist) to fill the budget ceiling — per flywheel discipline (budget is a ceiling, not a quota).
- **Run #23 totals:** 6 PRs merged (A.005.1/.2/.3 + the iter-state PRs + DR-026 read #66) + **2 open for review (#67 write adapter, #69 review adapter)**; audit-005 discharged; DR-025 discharged; M6 done; DR-037 prose fix; anchors refreshed.
- **HUMAN-GATED remaining (the only path forward):** (1) review+merge #67/#69; (2) the **ACTIVATION** PR (route-wire the live adapters + service-role creds + `pilot:false` + `publishEnabled`) = go-live; (3) **P1.C.4** needs the Whispering Willows **SoM prompt-set**; (4) pilot reviewer **seed** needs a provisioned pilot workspace; (5) live-Sandbox Tier-2/3 needs infra. (Low-pri dependency-free: `build:worker` dist wiring.)

## Attended SoM unblock (P1.C.4) — query-bank + spike + scaffolding — 2026-06-26
- **Query-bank APPROVED + merged [#71](https://github.com/digital-seniority/sagemark/pull/71):** 28 funnel-staged Whispering Willows prompts (awareness→consideration→decision→competitor→brand→retention), localized to Skagit County (Mount Vernon/Burlington/Anacortes/Sedro-Woolley); brand + nearby competitors confirmed from the live site (whisperingwillows.com). `som-prompt-set-whispering-willows.md`.
- **Feasibility spike done** (`som-feasibility-spike.md`): the channel matrix — Gateway model-API = a legal but PROXY signal (model answer, not consumer-engine citation; weakest for ChatGPT-consumer + Google AI Overviews); GEO-tracker vendors (Profound/AthenaHQ/Peec, ~$99-399/mo) = the real consumer-citation signal. Satisfies PR-021's "spike FIRST" gate.
- **HYBRID channel chosen (James)** → [[DR-038]] addendum: Claude = `direct-citation` (Gateway + web-search tool, real sources); ChatGPT/Gemini = `direct-proxy` (labeled, not a citation); GEO-tracker vendor = deferred seam. Per-row `source_channel` labeling is load-bearing (proxy never conflated with citation).
- **Scaffolding BUILT → [#72](https://github.com/digital-seniority/sagemark/pull/72) OPEN for review** (inert/`SOM_LIVE`-gated, judge 5/5·5/5): channel-agnostic `SomAdapter` + 28-prompt query-bank + `som-parse` + ingest/freshness crons (freshness=draft-only, structural) + `vercel.json` schedules + CRON_SECRET-gated handlers; Gateway-only (DR-013); tenancy-bound writes; `rollUpBySourceChannel` keeps proxy ≠ citation by type. 57 Tier-1 tests; 597 apps/seo pass. **INERT proof: SOM_LIVE unset ⇒ zero probes (non-vacuous test).**
- **P1.C.4 is BUILT but DoD-OPEN** — per the spec the DoD needs REAL rows from real creds/a contracted vendor (mocks insufficient). **Human-gated:** pick channel (done: hybrid) → supply provider keys via the Gateway + flip `SOM_LIVE` → a small wiring PR injects the live runner + store → real labeled rows land → DoD closes for the covered engines. GEO-tracker = later upgrade.
- **Phase-1 finish line:** merge #67 (write adapter) + #69 (review adapter) + #72 (SoM scaffolding), then the ACTIVATION (route-wire live adapters + creds + pilot:false + publishEnabled + SOM_LIVE) + the non-eng inputs (real credentialed reviewer, pilot workspace, live-Sandbox infra).

## Attended ACTIVATION staged — live pipeline merged + inert — 2026-06-26
- **Merged (James: "activate it"):** DR-026 write adapter [#67](https://github.com/digital-seniority/sagemark/pull/67) `d65e33b`, review adapter [#69](https://github.com/digital-seniority/sagemark/pull/69) `f75c6bf`, SoM subsystem [#72](https://github.com/digital-seniority/sagemark/pull/72) `b730efe`, **gated activation wiring [#74](https://github.com/digital-seniority/sagemark/pull/74) `533e751`** (judge 5/5·5/5; inert-by-default — mutation-tested; publishEnabled off, pilot:false-in-prod DR-037, Gateway-only, tenancy preserved; freshness correctly left inert).
- **The full live pipeline is now STAGED + INERT on preview.** Merging changed NOTHING live — `activation.ts` defaults every gate OFF. Phase 1: 12/12 mapped CODE landed; **0 open PRs.**
- **`go-live-checklist.md` written** — the exact env flip (`SUPABASE_*`/`AI_GATEWAY_API_KEY`/`SOM_LIVE`/`PUBLISH_ENABLED`) + the human prereqs (real credentialed reviewer replacing the placeholder per DR-037; pilot workspace) + recommended order. Reversible per-step.
- **Remaining (human / deferred, non-blocking):** the go-live env flip + reviewer + pilot workspace; freshness cron (awaits piece-id in the public projection); GEO-tracker vendor upgrade; live-Sandbox Stage B/C worker deploy. P1.C.4 DoD closes when real labeled SoM rows land.

## P1.C.4 live-smoke (user-authorized) + DoD runbook — 2026-06-26
- **Live SoM smoke PASSED** (`som-live-smoke-evidence.md`): real AI-Gateway calls across all 3 engines (no DB writes), replicating `runSomDirectProbe` exactly. **Claude (web-search/`direct-citation`) cites Whispering Willows for the discovery queries** ("best memory care in Skagit County", "Mount Vernon facilities") — the real north-star signal; **ChatGPT/Gemini (`direct-proxy`) only echo the brand when it's named** (no discovery citation). This validates the hybrid + the proxy≠citation labeling with LIVE data (not just mocks).
- **DoD-close runbook** added to `go-live-checklist.md` §"P1.C.4 DoD close": provision a real client (SQL) → set env (`SUPABASE_*`/`AI_GATEWAY_API_KEY`/`SOM_LIVE`/`CRON_SECRET`) → run the cron in the real runtime (local `pnpm dev` + curl, or the Vercel deploy) → verify rows with the given `share_of_model` query. **When real labeled rows land, P1.C.4 is DoD-complete.**
- Honest scope: the smoke proves the path; it does NOT persist rows (no provisioned client; I won't fabricate prod tenancy). The DB-write close is the human runbook above.

## P1.C.4 DoD CLOSED — real persisted SoM rows — 2026-06-26
