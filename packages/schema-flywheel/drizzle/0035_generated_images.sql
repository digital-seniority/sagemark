...
-- even on dedup so every spend is accounted).
--
-- STORAGE BUCKET IS OUT-OF-BAND — NOT IN THIS MIGRATION. The bytes live in a
-- PRIVATE bucket `seo-generated-images` (public=false). That bucket is created
-- OUT-OF-BAND (storage admin / Supabase MCP / dashboard), NOT here, because the
-- migration role on a real Supabase instance lacks CREATE/INSERT on the `storage`
-- schema — issuing `storage.*` DDL/DML in a migration fails with
-- `permission denied for schema storage` and takes the whole apply down. Bucket
-- provisioning is a storage-admin concern, not a SQL-migration concern. Create
-- `seo-generated-images` (id+name `seo-generated-images`, public OFF, no anon
-- storage policy) before the imagegen live path is enabled. The store uploads to
-- it at runtime; reads go through short-lived signed URLs (makeSupabaseSignUrl).
--
-- This migration is ADDITIVE-ONLY and idempotent (`IF NOT EXISTS`). It touches
-- ONLY the `public` schema. It MUST NOT alter or drop any existing table, column,
-- constraint, index, or policy. It follows the exact style of
-- 0033_content_clients_rls and 0032_release_records.
--
...
-- (The out-of-band `seo-generated-images` storage bucket is NOT touched by this
-- migration and is left in place; remove it via storage admin if a full teardown
-- is required.)
...
-- NOTE: the PRIVATE `seo-generated-images` storage bucket is intentionally NOT
-- created here — bucket provisioning is a storage-admin concern and the migration
-- role cannot write the `storage` schema (see header). Create it out-of-band.
