...
- **Files added/modified:** `apps/seo/src/worker/skills/load-suite.ts` (loads the `learnings/SKILLS/seo-copywriter/*` `SKILL.md` skills + points their kernel-host base URL at the `apps/seo` `/content/api/*` contract; PR 008 registers `seo-blog-writer`), `apps/seo/golden/whispering-willows/{pillar,spoke-*,faq,checklist}.json` (body + per-piece `clusterRole`/`funnelStage`/expected dimension scores/expected Stage-A verdict), `apps/seo/test/golden/regression.test.ts`, `apps/seo/test/acceptance/gate-spec.ts` (judge-prompt domain checks transcribed).
- **Acceptance criteria:**
...
- **Migration SQL (inline, `0033`):**
  ...
  CREATE TABLE share_of_model (
    ...
    source_channel  text NOT NULL DEFAULT 'direct',  -- direct | vendor-api (ToS fallback)
